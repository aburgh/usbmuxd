...
static int create_socket(void) {
	...
	if (listenfd == SOCKET_ERROR) {
		usbmuxd_log(LL_FATAL, "socket() failed: %d", WSAGetLastError());//strerror(errno));
		return -1;
	}
    
    memset(&bind_addr, 0, sizeof(bind_addr));
	bind_addr.sin_family = AF_INET;
    bind_addr.sin_port = USBMUXD_SOCKET_PORT;
...
#elif defined(WIN32)
static int ppoll(struct pollfd *fds, int nfds, const struct timespec *timeout, const sigset_t *sigmask)
{
	int ready;
	// sigset_t origmask;
	int to = (int) (timeout->tv_sec*1000 + timeout->tv_nsec/1000000);
...
int main(int argc, char *argv[])
{
    ...
    if (res != 0) {
		usbmuxd_log(LL_FATAL, "WSAStartup failed: %d\n", res);
        return -1;
    }
#else
